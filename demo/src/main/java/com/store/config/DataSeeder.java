...
public class DataSeeder implements CommandLineRunner {
    ...
    private void seedPermissionsAndRoles() {
        ...
        Set<Permission> staffPermissions = allPermissions.stream()
                .filter(p -> !p.getPermissionCode().equals("ROLE_MANAGE")
                        && !p.getPermissionCode().equals("SETTING_MANAGE"))
    ...
    private void seedAttributesAndSpecifications() {
        ...
        boolean hasCpuSocket = categoryRepository.findBySlug("cpu-bo-vi-xu-ly")
                .map(cat -> attributeRepository.existsByCategoryCategoryIdAndNameIgnoreCase(cat.getCategoryId(),
                        "Socket"))
                .orElse(false);
        ...
        for (Product p : products) {
            ...
            if (catSlug.contains("cpu")) {
                if (attrs.containsKey("Socket")) {
                    String socket = nameLower.contains("intel") || nameLower.contains("i9") || nameLower.contains("i7")
                            || nameLower.contains("i5") ? "LGA1700" : "AM5";
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Socket")).value(socket)
                            .build());
                }
                if (attrs.containsKey("Số nhân")) {
                    String cores = nameLower.contains("i9") ? "24"
                            : (nameLower.contains("i7") ? "20"
                                    : (nameLower.contains("i5") ? "14" : (nameLower.contains("7950x") ? "16" : "8")));
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Số nhân")).value(cores)
                            .build());
                }
                if (attrs.containsKey("Số luồng")) {
                    String threads = nameLower.contains("i9") ? "32"
                            : (nameLower.contains("i7") ? "28" : (nameLower.contains("i5") ? "20" : "16"));
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Số luồng"))
                            .value(threads).build());
                }
                if (attrs.containsKey("Xung nhịp Turbo tối đa")) {
                    String boost = nameLower.contains("14900k") ? "6.0 GHz"
                            : (nameLower.contains("14700k") ? "5.6 GHz"
                                    : (nameLower.contains("7950x") ? "5.7 GHz" : "5.4 GHz"));
                    pavList.add(ProductAttributeValue.builder().product(p)
                            .attribute(attrs.get("Xung nhịp Turbo tối đa")).value(boost).build());
                }
                if (attrs.containsKey("TDP")) {
                    String tdp = nameLower.contains("k") || nameLower.contains("x") ? "125" : "65";
                    pavList.add(
                            ProductAttributeValue.builder().product(p).attribute(attrs.get("TDP")).value(tdp).build());
                }
            }
            // VGA
            else if (catSlug.contains("vga")) {
                if (attrs.containsKey("Dung lượng VRAM")) {
                    String vram = nameLower.contains("4090") ? "24GB"
                            : (nameLower.contains("4080") ? "16GB" : (nameLower.contains("4070") ? "12GB" : "8GB"));
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Dung lượng VRAM"))
                            .value(vram).build());
                }
                if (attrs.containsKey("Chuẩn bộ nhớ")) {
                    String mem = nameLower.contains("40") ? "GDDR6X" : "GDDR6";
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Chuẩn bộ nhớ"))
                            .value(mem).build());
                }
                if (attrs.containsKey("Công suất nguồn đề xuất")) {
                    String psu = nameLower.contains("4090") ? "1000"
                            : (nameLower.contains("4080") ? "850" : (nameLower.contains("4070") ? "750" : "650"));
                    pavList.add(ProductAttributeValue.builder().product(p)
                            .attribute(attrs.get("Công suất nguồn đề xuất")).value(psu).build());
                }
                if (attrs.containsKey("Cổng xuất hình")) {
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Cổng xuất hình"))
                            .value("3x DisplayPort 1.4a, 1x HDMI 2.1a").build());
                }
            }
            // RAM
                ...
                if (attrs.containsKey("Chuẩn RAM")) {
                    ...
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Chuẩn RAM")).value(type)
                            .build());
                }
                if (attrs.containsKey("Dung lượng")) {
                    String cap = nameLower.contains("64gb") ? "64GB (2x32GB)"
                            : (nameLower.contains("32gb") ? "32GB (2x16GB)" : "16GB (2x8GB)");
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Dung lượng")).value(cap)
                            .build());
                }
                if (attrs.containsKey("Bus RAM")) {
                    String bus = nameLower.contains("6000") ? "6000MHz"
                            : (nameLower.contains("5600") ? "5600MHz" : "3200MHz");
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Bus RAM")).value(bus)
                            .build());
                }
            }
            // Laptop & MacBook
                ...
                if (attrs.containsKey("Kích thước màn hình")) {
                    String size = nameLower.contains("16") || nameLower.contains("g16") ? "16 inch"
                            : (nameLower.contains("14") ? "14 inch"
                                    : (nameLower.contains("13") ? "13.6 inch" : "15.6 inch"));
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Kích thước màn hình"))
                            .value(size).build());
                }
                if (attrs.containsKey("Độ phân giải & Tần số quét")) {
                    String res = nameLower.contains("rog") || nameLower.contains("legion") ? "QHD+ 240Hz 100% DCI-P3"
                            : (nameLower.contains("macbook") ? "Liquid Retina XDR ProMotion 120Hz" : "FHD 144Hz IPS");
                    pavList.add(ProductAttributeValue.builder().product(p)
                            .attribute(attrs.get("Độ phân giải & Tần số quét")).value(res).build());
                }
                if (attrs.containsKey("Bộ vi xử lý (CPU)")) {
                    String cpu = nameLower.contains("macbook") ? "Apple M3 Pro (12-Core CPU)"
                            : (nameLower.contains("i9") ? "Intel Core i9-14900HX" : "Intel Core i7-14700HX");
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Bộ vi xử lý (CPU)"))
                            .value(cpu).build());
                }
                if (attrs.containsKey("Card đồ họa (GPU)")) {
                    String gpu = nameLower.contains("macbook") ? "Apple M3 Pro 18-Core GPU"
                            : (nameLower.contains("4080") ? "NVIDIA GeForce RTX 4080 12GB"
                                    : (nameLower.contains("4070") ? "NVIDIA GeForce RTX 4070 8GB"
                                            : "NVIDIA GeForce RTX 4060 8GB"));
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Card đồ họa (GPU)"))
                            .value(gpu).build());
                }
                if (attrs.containsKey("Trọng lượng")) {
                    String weight = nameLower.contains("macbook") ? "1.24 kg"
                            : (nameLower.contains("rog") || nameLower.contains("legion") ? "2.4 kg" : "1.8 kg");
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Trọng lượng"))
                            .value(weight).build());
                }
            }
            // Monitor
                ...
                if (attrs.containsKey("Kích thước")) {
                    String size = nameLower.contains("32") ? "32 inch"
                            : (nameLower.contains("24") ? "24 inch" : "27 inch");
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Kích thước"))
                            .value(size).build());
                }
                if (attrs.containsKey("Độ phân giải")) {
                    String res = nameLower.contains("4k") ? "4K UHD (3840x2160)"
                            : (nameLower.contains("2k") || nameLower.contains("qhd") ? "2K QHD (2560x1440)"
                                    : "FHD (1920x1080)");
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Độ phân giải"))
                            .value(res).build());
                }
                if (attrs.containsKey("Tấm nền")) {
                    String panel = nameLower.contains("oled") ? "QD-OLED" : "Fast IPS";
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Tấm nền")).value(panel)
                            .build());
                }
                if (attrs.containsKey("Tần số quét")) {
                    String hz = nameLower.contains("240") ? "240Hz"
                            : (nameLower.contains("180") ? "180Hz" : (nameLower.contains("165") ? "165Hz" : "144Hz"));
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Tần số quét")).value(hz)
                            .build());
                }
                if (attrs.containsKey("Chuẩn màu")) {
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Chuẩn màu"))
                            .value("99% DCI-P3, Delta E < 2").build());
                }
            }
            // SSD
            else if (catSlug.contains("ssd")) {
                if (attrs.containsKey("Chuẩn kết nối")) {
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Chuẩn kết nối"))
                            .value("M.2 NVMe PCIe 4.0 x4").build());
                }
                if (attrs.containsKey("Tốc độ đọc tối đa")) {
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Tốc độ đọc tối đa"))
                            .value("7450 MB/s").build());
                }
                if (attrs.containsKey("Tốc độ ghi tối đa")) {
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Tốc độ ghi tối đa"))
                            .value("6900 MB/s").build());
                }
            }
            // Bàn phím
            else if (catSlug.contains("ban-phim")) {
                if (attrs.containsKey("Loại kết nối")) {
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Loại kết nối"))
                            .value("Wireless 2.4GHz / Bluetooth 5.1 / Type-C").build());
                }
                if (attrs.containsKey("Loại Switch")) {
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Loại Switch"))
                            .value("Linear Hot-swappable Pre-lubed").build());
                }
                if (attrs.containsKey("Layout")) {
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Layout"))
                            .value("75% (81 Phím)").build());
                }
                if (attrs.containsKey("LED RGB")) {
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("LED RGB"))
                            .value("RGB 16.8 triệu màu Per-Key").build());
                }
            }
            // Chuột
            else if (catSlug.contains("chuot")) {
                if (attrs.containsKey("Cảm biến (Sensor)")) {
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Cảm biến (Sensor)"))
                            .value("Optical Gaming Sensor 30K").build());
                }
                if (attrs.containsKey("DPI tối đa")) {
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("DPI tối đa"))
                            .value("30000").build());
                }
                if (attrs.containsKey("Trọng lượng")) {
                    pavList.add(ProductAttributeValue.builder().product(p).attribute(attrs.get("Trọng lượng"))
                            .value("58g siêu nhẹ").build());
                }
            }
        }
