...
describe('RoleManageComponent', () => {
  ...
  const mockGroups: PermissionGroup[] = [
    {
      ...
      groupName: 'Sản Phẩm',
      permissions: [
        { permissionId: 1, permissionCode: 'PRODUCT_VIEW', description: 'Xem SP', moduleGroup: 'PRODUCT' },
        { permissionId: 2, permissionCode: 'PRODUCT_CREATE', description: 'Tạo SP', moduleGroup: 'PRODUCT' }
      ]
    },
    {
      groupCode: 'USER',
      groupName: 'Nhân Sự & Phân Quyền',
      permissions: [
  ...
  it('should create and load roles and permissions matrix on init', () => {
  ...
  it('should switch tabs between roles and matrix', () => {
    component.switchTab('matrix');
  ...
  it('should delete custom role with 0 users', () => {
    component.openConfirmDelete(mockRoles[2]); // custom role, 0 users
  ...
  it('should not open delete modal for system role or role with users', () => {
    component.openConfirmDelete(mockRoles[0]); // ROLE_ADMIN is system role
    expect(component.isConfirmDeleteOpen()).toBe(false);

    component.openConfirmDelete(mockRoles[1]); // ROLE_STAFF has userCount 5
    expect(component.isConfirmDeleteOpen()).toBe(false);
  });

  it('should toggle matrix permission and mark dirty', () => {
    expect(component.isMatrixDirty()).toBe(false);

    component.toggleMatrixPermission(2, 'PRODUCT_CREATE'); // ROLE_STAFF adds PRODUCT_CREATE

    expect(component.matrixDraft().get(2)?.has('PRODUCT_CREATE')).toBe(true);
    expect(component.isMatrixDirty()).toBe(true);
  });

  it('should prevent toggling critical admin permissions on ROLE_ADMIN', () => {
    ...
    expect(component.matrixDraft().get(1)?.has('ROLE_MANAGE')).toBe(true); // Remains granted
  });

  it('should save matrix changes across roles and reload', () => {
    component.toggleMatrixPermission(2, 'PRODUCT_CREATE');
    expect(component.isMatrixDirty()).toBe(true);

    component.saveMatrixChanges();

    expect(roleServiceMock.updateRolePermissions).toHaveBeenCalled();
  });
});
